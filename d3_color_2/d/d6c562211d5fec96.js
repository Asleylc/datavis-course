...
d3.schemeGnBu[7]
  ...
  main.variable(observer("colorScale")).define("colorScale", ["d3","greens"], function(d3,greens){return(
d3.scaleQuantize()
    .domain([0,140])
    .range(greens)
)});
  main.variable(observer("markers")).define("markers", function(){return(
[0,50,100,200,300]
)});
...
function zoomToFeature(e) {
  ...
  main.variable(observer("info")).define("info", ["L","homicidios_por_populacao","map"], function(L,homicidios_por_populacao,map)
{
  // control that shows state info on hover
	...
	infoControl.update = function (feat) {
			this._div.innerHTML = '<h5>Número de homicídios</h5>' +  (feat ?
				'<b>' 
              + homicidios_por_populacao.get(feat.properties.NOME)[1] 
              + '  por 100 mil pessoas.': 'Passe o mouse sobre o bairro');
	}
  ...
  main.variable(observer("homicidios_por_populacao")).define("homicidios_por_populacao", ["d3"], function(d3){return(
d3.csv("https://gist.githubusercontent.com/emanueles/8ee225792454a78151d0842a10642a29/raw/9984e32effa99da3cea2ec87a7c9b7b231c71d3c/fortaleza_crimes_populacao.csv").then(function(data){
  ...
  data.forEach(function(d){
      let homicidiosPorPopulacao;
        if(d.Populacao == -1)
          homicidiosPorPopulacao = 0
        else
          homicidiosPorPopulacao = (( 100000/ d.Populacao)*d.Homicidios).toFixed(2)
            Map.set(d.Bairro,[+d.Homicidios, homicidiosPorPopulacao])
        })    
...
d3.csv("https://gist.githubusercontent.com/emanueles/bae10fc42b13886b5a00b79737f2b50d/raw/ffb35abe6d9ba0c4ce4866b827f1916748ab65ef/homicidios_2012.csv").then(function(data) {
  ...
  main.variable(observer("style")).define("style", ["colorScale","homicidios_por_populacao"], function(colorScale,homicidios_por_populacao){return(
function style(feature) {
		 return {
					...
					dashArray: '3',
					fillOpacity: 0.6,
					fillColor: colorScale(homicidios_por_populacao.get(feature.properties.NOME)[1])
				};
	}
)});
